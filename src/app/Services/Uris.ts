export class Uris {
    public static API_ENDPOINT = "http://159.89.49.19/api/v1/";
    public static API_LOGIN = `${Uris.API_ENDPOINT}login/`;
    public static API_AGRONODO_POST = `${Uris.API_ENDPOINT}register/adminagronodo/`
    public static API_AGRONODO_GET = `${Uris.API_ENDPOINT}register/adminagronodo/`;
    
    public static API_AGROINDUSTRIA_AGRONODO_POST = `${Uris.API_ENDPOINT}register/agroindustria/`;

}